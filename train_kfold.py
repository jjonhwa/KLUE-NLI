...
def train_baseline_kfold(dataset, args):
    stf = StratifiedKFold(
        n_splits=args.n_splits, shuffle=True, randome_state=seed_everything(42)
    )

    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    for fold, (train_idx, valid_idx) in enumerate(stf.split(dataset, list(dataset['label']))):
        print(f"Fold {fold+1}")
        ...
        # Dataset
        train_data = NLI_Dataset(train_dataset, tokenizer, args.max_length)
        valid_data = NLI_Dataset(valid_dataset, tokenizer, args.max_length)
        ...
        torch.save(model, f'{args.save_path}baseline_fold{fold+1}.pt')
        
        model.cpu()
        del model
        torch.cuda.empty_cache()
        
def train_explain_kfold(dataset, args):
    stf = StratifiedKFold(
        n_splits=args.n_splits, shuffle=True, randome_state=seed_everything(42)
    )
    ...
    for fold, (train_idx, valid_idx) in enumerate(stf.split(dataset, list(dataset['label']))):
        ...
        # Dataset
        train_data = NLI_Explain_Dataset(train_dataset, tokenizer, args.max_length)
        valid_data = NLI_Explain_Dataset(valid_dataset, tokenizer, args.max_length)
        ...
        if not os.path.isdir(args.save_path):
            os.mkdir(args.save_path)
        torch.save(model, f'{args.save_path}explaining_fold{fold+1}.pt')
...
if __name__ == "__main__":
    ...
    # -- dataset
    parser.add_argument('--n_splits', type=int, default=5, help="number of fold (default: 5)")
    ...
    if args.mode == 'baseline':
        train_baseline_kfold(train, args)
    elif args.mode == 'explain':
        train_explain_kfold(train, args)
